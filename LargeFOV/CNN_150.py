...
from keras.layers import Conv2D, Dense, Dropout, Flatten, MaxPool2D, GaussianNoise, BatchNormalization, SpatialDropout2D
...
model.add(Conv2D(int(2*ConvScale), (3,3), padding='valid', input_shape=input_shape, activity_regularizer = regularizers.l2(reg_scale)))
...
model.add(Conv2D(int(2*ConvScale), (3,3), padding='valid', activity_regularizer = regularizers.l2(reg_scale)))
model.add(LeakyReLU(alpha = alpha))
model.add(SpatialDropout2D(spatial_d_rate))
...
model.add(Conv2D(int(ConvScale), (3,3), padding='valid', activity_regularizer = regularizers.l2(reg_scale)))
model.add(LeakyReLU(alpha = alpha))
model.add(SpatialDropout2D(spatial_d_rate))
# model.add(GaussianNoise(GN3))
model.add(Conv2D(int(ConvScale), (3,3), padding='valid', activity_regularizer = regularizers.l2(reg_scale)))
...
model.add(Conv2D(int(ConvScale), (3, 3), padding = 'valid', activation = 'relu', activity_regularizer = regularizers.l2(reg_scale)))
model.add(SpatialDropout2D(spatial_d_rate))
...
intermediate_output = intermediate_layer_model.predict(np.reshape(TestYes[12], (1, 150, 150, 1)))
plt.subplot(4,3,7)
plt.imshow(TestYes[12])
plt.axis('off')
plt.subplot(4,3,8)
plt.imshow(np.reshape(intermediate_output[:, :, :, 20], (intermediate_output.shape[1], intermediate_output.shape[2])))
plt.axis('off')
